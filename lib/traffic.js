...
function classifyExisting(db) {
  ...
  // One UPDATE per distinct UA; without this index each is a full table scan.
  db.exec('CREATE INDEX IF NOT EXISTS traffic_ua_class ON traffic(ua, client_class)');
...
function initTraffic(db) {
  ...
  // classifyExisting's subresource check is a correlated lookup by (ip, ua); without this
  // index it degrades to a nested full-table scan and can peg the CPU for minutes on a
  // traffic table with tens of thousands of rows (real-world scanner/bot traffic included).
  db.exec('CREATE INDEX IF NOT EXISTS traffic_ip_ua_ts ON traffic(ip, ua, ts)');
